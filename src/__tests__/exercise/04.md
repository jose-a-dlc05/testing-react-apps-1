...
// What type of data goes into handleSubmit when submit button is clicked for
toHaveBeenCalledWith? // i.e. - object, array, primitive type of data? //
Sometimes you have to consider what could go wrong. In this example, it's
clicking on submit twice, so we have to make sure that it only goes once.
